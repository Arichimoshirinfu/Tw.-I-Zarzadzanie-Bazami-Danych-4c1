...
<body>
	...
	$wyn_dep = mysqli_query($id_conn, $sql_dep);
	    ...
	    $depts = $depts . ', ' . $w_dep['id'];			
	}
	if ($ok_dep==0) {
	    echo 'Nie ma departamentu "' . $did . '", wybierz jeden z następujących: ' . substr($depts,1);
            mysqli_close($id_conn);
            exit;   
	}
        
        ##### MANAGER ##### 
        $sql_man = "SELECT emp.id FROM emp;";
	$wyn_man = mysqli_query($id_conn, $sql_man);
        if (mysqli_errno($id_conn)) 
        {
            echo "Błąd w zapytania o potencjalnego managera: " . $baza . ' (' . mysqli_error($id_conn) . ')';
	    mysqli_close($id_conn);
            exit;
        }
	...
	$wid_do = $w_wid['salary_max'];       
        ...
        ######################
        $sql_ins = "INSERT INTO emp (id, first_name, last_name, username, title, dept_id, 
